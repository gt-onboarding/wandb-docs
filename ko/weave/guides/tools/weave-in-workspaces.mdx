...
description: "Weave 추적을 W&B 학습 실행과 통합하여, 사용자 정의 가능한 워크스페이스 대시보드에서 기존 ML 지표와 함께 함수 실행 세부 정보와 진단 정보를 캡처해 학습 중 모델 동작에 대한 깊은 가시성을 확보할 수 있습니다."
---

<div id="log-traces-during-model-training-runs">
  # 모델 학습 실행 중 트레이스 로그 기록하기
</div>
...
ML 파이프라인에서 [함수에 `@weave.op` 데코레이터를 적용](/ko/weave/quickstart)하면, 해당 함수들의 실행 정보를 자동으로 수집하고 개인 워크스페이스에서 확인할 수 있습니다.
...
1. W&amp;B UI를 열고 **Projects** 탭을 클릭합니다. 그러면 사용자의 프로젝트 목록이 열립니다.
2. 프로젝트 목록에서 실행을 기록한 프로젝트를 클릭합니다. 그러면 **Workspaces** 페이지가 열립니다.
3. 워크스페이스를 자동 워크스페이스로 설정한 경우, 워크스페이스에는 실행에 대한 데이터 시각화와 정보가 자동으로 채워집니다. 실행에 대한 트레이스 데이터는 워크스페이스의 **Weave** 섹션에 있습니다. 워크스페이스가 수동 워크스페이스인 경우, **Add panels**를 클릭하고 **Add panels** 메뉴의 **Weave** 섹션에서 새 패널을 선택하여 Weave 패널을 추가할 수 있습니다.
...
Weave 트레이스에서 [W&amp;B 아티팩트](/ko/models/artifacts) (예: 모델, 데이터셋, 체크포인트)를 참조할 수 있습니다. 이렇게 하면 Weave UI에 아티팩트 상세 페이지로 바로 이동하는 클릭 가능한 링크가 생성되어, 특정 작업에서 어떤 아티팩트 버전이 사용되었는지 추적하는 데 도움이 됩니다.

예를 들어, 두 개의 모델 버전(`v1`과 `v2`)을 학습했다면, 해당 모델을 조회할 때 `v1` 아티팩트 참조를 트레이스의 속성으로 추가할 수 있습니다. 이렇게 하면 어떤 아티팩트 버전이 특정 출력값을 생성했는지 정확히 추적할 수 있습니다.
...
`Model`의 속성으로 아티팩트를 추가하려면 모델 인스턴스를 생성할 때 아티팩트 URL을 전달하세요:
