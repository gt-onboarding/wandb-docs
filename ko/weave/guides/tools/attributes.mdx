...
Weave의 속성(attributes)을 사용하면 트레이스와 평가에 사용자 정의 메타데이터를 추가할 수 있습니다. 이 메타데이터에는 환경 이름, 모델 버전, 실험 ID, 사용자 ID와 같이 Weave 데이터를 구성, 필터링, 분석하는 데 도움이 되는 다양한 컨텍스트 정보가 포함될 수 있습니다.

Weave에서는 속성을 추가하는 두 가지 방법을 제공합니다:

* **호출 단위 속성(Per-call attributes)**: `weave.attributes()`를 사용하여 특정 연산 또는 코드 블록에 메타데이터를 추가합니다.
* **전역 속성(Global attributes)**: 초기화 시 `global_attributes` 필드를 사용하여 프로젝트의 모든 트레이스와 평가에 적용되는 속성을 설정합니다.

트레이싱 및 평가 과정에서 로깅된 모든 속성은 UI에서 확인할 수 있습니다. 이후 이 속성을 사용하여 데이터를 필터링하고 그룹화할 수 있습니다.

<Tip>
`call.attributes`는 호출이 시작된 이후에는 수정할 수 없습니다. op를 호출하기 전에 이 컨텍스트 관리자를 사용해 필요한 메타데이터를 설정하세요.
</Tip>
...
`weave.attributes()` 컨텍스트 매니저를 사용하면 특정으로 추적되는 연산에 메타데이터를 추가할 수 있습니다. 이를 통해 개별 함수 호출이나 평가 실행에 맥락 정보를 태그로 달 수 있습니다.
...
이 함수는 컨텍스트 매니저 블록(Python) 또는 콜백 함수(TypeScript) 안에 있는 모든 추적된 연산에 속성을 적용합니다.

또한 `weave.attributes()` 컨텍스트를 중첩해서 사용할 수도 있습니다. 내부 컨텍스트는 동일한 키에 대해 외부 컨텍스트를 덮어씁니다:
...
Weave를 초기화할 때 전역 속성을 설정하면, 해당 속성은 프로젝트 내 **모든** trace와 evaluation에 자동으로 적용됩니다. 이는 환경, 배포 버전, 팀 정보와 같은 프로젝트 전역 메타데이터를 전파하는 데 유용합니다.
...
# 이제 global_attributes 딕셔너리에 정의된 속성들이 이후 모든 연산에 적용됩니다
@weave.op
def my_function():
    return "Hello"

my_function()  # 모든 전역 속성을 자동으로 포함합니다

# evaluation에도 전역 속성이 포함됩니다
...
async function main() {
    ...
    // 이제 globalAttributes 객체에 정의된 속성들이 이후 모든 연산에 적용됩니다
    const myFunction = op(async function myFunction() {
        return 'Hello';
    });
    
    const result = await myFunction();  // 모든 전역 속성을 자동으로 포함합니다
    console.log('Result:', result);
}
main().catch(console.error);
```
...
# 이 호출의 속성: env='production', app_version='2.1.0'
process("test1")

# 이 호출의 속성: env='staging', app_version='2.1.0', experiment='A'
...
현재 호출에 기록된 속성 집합을 반환합니다. 이는 호출을 디버깅하거나 조건부 로직을 위한 컨텍스트를 확인하는 데 도움이 됩니다.

<Tabs>
<Tab title="Python">

다음 예시는 `process_data` 함수를 로깅하도록 Weave 데코레이터를 설정하고, 로깅할 속성을 구성한 뒤, 실행 시 해당 속성을 확인합니다.
...
<Note>
`weave.get_current_call()`은 `@weave.op`으로 데코레이팅된 함수 **내부에서만** 동작합니다. op 바깥에서는 `None`을 반환합니다.
</Note>
</Tab>
<Tab title="TypeScript">

Weave TypeScript SDK를 사용하면 `client.getCurrentAttributes()`로 현재 속성을 가져올 수 있습니다. Weave Python SDK와 달리, TypeScript에서는 op 내부뿐 아니라 `withAttributes()` 컨텍스트 내 어디에서든 속성에 접근할 수 있습니다.
