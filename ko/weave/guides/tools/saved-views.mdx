---
title: "저장된 뷰 생성 및 관리"
description: "추적된 함수 호출 및 평가와 상호작용하는 방식을 사용자 지정합니다"
---

Weave에서 _저장된 뷰(Saved Views)_ 를 사용하면 추적된 함수 호출 및 평가와 상호작용하는 방식을 사용자 지정할 수 있습니다. 저장된 뷰를 정의하면 필터, 정렬, 열 표시 여부를 구성하여 관련 데이터를 빠르게 확인할 수 있습니다.
...
- [Python SDK에서 저장된 뷰(Saved Views)를 생성하고 수정하는 방법](#saved-views-in-the-python-sdk).
- [Weave UI에서 저장된 뷰(Saved Views)를 생성하고 사용하는 방법](#saved-views-in-the-ui).

<div id="saved-views-in-the-python-sdk">
  ## Python SDK의 Saved view
</div>
...
`.to_grid()`를 사용해 `SavedView`를 그리드 형태로 표시합니다. `limit`으로 표시할 최대 행 개수를 지정합니다.
...
<div id="filter-by-operation-name">
  ### operation 이름으로 필터링하기
</div>

Weave에서는 모든 trace 또는 eval이 operation 이름과 연결되어 있습니다.
특정 operation이 실행된 호출만 포함하도록 `SavedView`를 필터링하려면 `.filter_op()`을(를) 사용하세요.
...
`.add_filter()`를 사용해 뷰에 사용자 지정 필터를 적용하세요. [지원되는 필터 연산자](#filter-operators) 중 하나와 조건을 사용해 필터를 정의하세요.
...
`.get_calls()`를 사용하여 저장된 뷰에 설정된 필터와 일치하는 함수 호출을 가져옵니다. `limit`과 `offset` 같은 옵션 매개변수를 지정할 수 있습니다.
...
Weave UI에서 저장된 뷰를 만들고 불러오고 이름을 바꾸고 수정할 수 있습니다. 더 정밀하게 제어하려면 [Python SDK](#saved-views-in-the-python-sdk)를 사용하세요.
...
3. 다음 두 가지 방법 중 하나로 뷰를 저장합니다:
   ...
   - **Save view** 왼쪽에 있는 햄버거 메뉴를 클릭한 후, 드롭다운 메뉴에서 **+ Save as new view**를 클릭합니다.
...
1. **Traces** 또는 **Evals** 탭으로 이동합니다.
2. 탭 제목 왼쪽에 있는 햄버거 메뉴를 클릭합니다. 모든 저장된 뷰가 나열된 드롭다운 메뉴가 표시됩니다.
3. 열고자 하는 뷰를 클릭합니다. 선택한 저장된 뷰가 **Traces** 또는 **Evals** 탭에 표시됩니다. 
...
1. [저장된 뷰 로드하기](#load-a-saved-view)에 설명된 단계를 따르세요.
2. 테이블 구성을 원하는 대로 수정하세요.
3. 화면 오른쪽 상단에서 **Save view**를 클릭하세요. 
...
1. **Traces** 또는 **Evals** 탭으로 이동합니다.
2. 삭제하려는 [보기를 불러옵니다](#load-a-saved-view).
3. **Save view** 왼쪽의 햄버거 메뉴를 클릭합니다. 
4. 드롭다운 메뉴에서 **Delete view**를 클릭합니다.
5. 팝업 창에서 **Delete view**를 클릭해 삭제를 확인합니다. 삭제를 취소하려면 **Cancel**을 클릭합니다.
...
1. **Traces** 또는 **Evals** 탭으로 이동합니다.
2. **Traces** 또는 **Evals** 탭 오른쪽에 있는 햄버거 메뉴를 클릭합니다. 저장된 모든 보기가 표시되는 드롭다운 메뉴가 나타납니다. 
