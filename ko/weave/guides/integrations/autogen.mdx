---
title: "AutoGen"
description: "Weave를 사용하여 AutoGen 에이전트와 멀티 에이전트 시스템을 추적하고 모니터링하기"
---

{/* TODO: 이 가이드용 Colab 노트북을 추가하고 여기 배지 링크를 넣으세요. */}

AutoGen은 Microsoft에서 개발한, AI 에이전트와 애플리케이션을 구축하기 위한 프레임워크입니다. 복잡한 멀티 에이전트 시스템을 쉽게 만들 수 있도록 대화형 AI용 구성 요소(`AgentChat`), 핵심 멀티 에이전트 기능(`Core`), 외부 서비스 통합(`Extensions`)을 제공합니다. 또한 AutoGen은 코드 작성 없이 에이전트를 프로토타이핑할 수 있는 `Studio`도 제공합니다. 자세한 내용은 [공식 AutoGen 문서](https://microsoft.github.io/autogen/stable//index.html)를 참고하세요.

<Note>
  이 가이드는 독자가 [AutoGen](https://microsoft.github.io/autogen/stable/index.html)에 대한 기본적인 이해가 있다고 가정합니다.
</Note>

Weave는 [AutoGen](https://microsoft.github.io/autogen/stable/index.html)과 통합되어 멀티 에이전트 애플리케이션의 실행 과정을 추적하고 시각화하는 데 도움을 줍니다. Weave를 초기화하기만 하면 `autogen_agentchat`, `autogen_core`, `autogen_ext` 내의 상호작용을 자동으로 추적할 수 있습니다. 이 가이드에서는 Weave를 AutoGen과 함께 사용하는 다양한 예제를 살펴봅니다.

<div id="prerequisites">
  ## 사전 준비 사항
</div>

시작하기 전에 AutoGen 및 Weave가 설치되어 있는지 확인하세요. 사용하려는 LLM 제공업체에 해당하는 SDK(예: OpenAI, Anthropic)도 필요합니다.
...
트레이스를 수집하기 시작하려면 스크립트 맨 앞에서 Weave를 초기화하세요.
...
Weave는 AutoGen 내에서 모델 클라이언트에 직접 이루어지는 호출을 트레이싱할 수 있습니다.

<div id="tracing-a-client-create-call">
  ### 클라이언트 생성 호출 추적
</div>

이 예제는 `OpenAIChatCompletionClient`에 대한 한 번의 호출을 추적하는 방법을 보여줍니다.
...
AutoGen의 `ChatCompletionCache`를 사용할 수 있으며, Weave는 이러한 상호작용을 추적하여 응답이 캐시에서 반환된 것인지, 새로운 호출에서 생성된 것인지 보여줍니다.
...
Weave는 에이전트와 해당 도구 사용을 추적하여, 에이전트가 어떤 도구를 선택하고 실행하는지 볼 수 있게 해줍니다.
...
`RoundRobinGroupChat`과 같은 그룹 채팅 내 상호작용은 Weave에서 추적하므로, 에이전트 간 대화 흐름을 추적할 수 있습니다.
...
AutoGen의 메모리 구성 요소는 Weave로 추적할 수 있습니다. 가독성을 높이기 위해 `@weave.op()`을 사용하여 메모리 연산을 하나의 트레이스로 묶을 수 있습니다.
...
문서 인덱싱과 `ChromaDBVectorMemory` 같은 메모리 시스템을 사용한 검색을 포함하는 Retrieval Augmented Generation (RAG) 워크플로는 추적할 수 있습니다. RAG 프로세스에 `@weave.op()`을 데코레이터로 감싸면 전체 흐름을 시각화하는 데 도움이 됩니다。

<Note>
  이 RAG 예제에는 `chromadb`가 필요합니다. `pip install chromadb`로 설치하세요.
</Note>
...
Weave는 `SingleThreadedAgentRuntime`와 같은 AutoGen의 에이전트 런타임 내부에서 실행되는 연산을 추적할 수 있습니다. 런타임 실행 함수를 `@weave.op()`으로 감싸면 관련된 추적들을 하나의 그룹으로 묶을 수 있습니다.
...
에이전트 상호작용의 순서를 정의하는 복잡한 에이전트 워크플로도 추적할 수 있습니다. `@weave.op()`을 사용해 전체 워크플로에 대한 고수준 추적을 남길 수 있습니다.
...
Weave는 AutoGen 에이전트가 코드 생성 및 실행을 수행하는 과정을 추적합니다.
...
이 가이드는 Weave를 AutoGen과 통합하기 위한 출발점을 제공합니다. Weave UI를 탐색하여 에이전트 상호작용, 모델 호출, 도구 사용 내역에 대한 상세한 추적 정보를 확인하십시오.