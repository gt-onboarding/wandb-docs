---
title: "Not Diamond ¬◇"
description: "Not Diamond를 사용해 정확도를 극대화하고 비용을 절감하면서, 요구 사항에 가장 적합한 모델로 프롬프트를 라우팅하세요"
---

복잡한 LLM 워크플로우를 구축할 때는 정확도, 비용, 호출 지연 시간 등에 따라 서로 다른 모델에 프롬프트를 전달해야 할 수 있습니다. 이때 [Not Diamond][nd]를 사용하면 워크플로우 내 프롬프트를 요구 사항에 가장 적합한 모델로 라우팅하여 모델 비용을 절감하면서 정확도를 최대화할 수 있습니다.

<div id="getting-started">
  ## 시작하기
</div>

먼저 [계정을 생성][account]하고 [API key를 발급][keys]한 다음, 이를 `NOTDIAMOND_API_KEY` 환경 변수로 설정합니다.

![[Create an API key](/ko/weave/guides/integrations/imgs/notdiamond/api-keys.png)]

이제 다음 작업을 수행할 수 있습니다:

* [퀵스타트 가이드][quickstart guide]를 시도하거나,
...
워크플로 시작 시 `weave.init()`만 실행한 다음, 이후에는 평소처럼 라우팅된
...
[Evaluations][evals]에서 자체 [커스텀 라우터]를 학습해, Not Diamond가 특정 사용 사례에 맞춰
...
이 preference ID를 모든 `model_select` 요청에서 재사용하면, 평가 데이터에 대해
성능을 최대화하고 비용을 최소화하도록 프롬프트를 라우팅할 수 있습니다:
...
추가 지원이 필요하면 [docs]를 방문하거나 [support]로 메시지를 보내세요.
