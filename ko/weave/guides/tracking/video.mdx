...
Weave는 [`moviepy`](https://zulko.github.io/moviepy/)를 사용해 비디오를 자동으로 로깅합니다. 이를 통해 트레이싱된 함수에 비디오 입력과 출력을 전달할 수 있으며, Weave가 비디오 데이터 업로드와 저장을 자동으로 처리합니다.
...
클립이 `VideoFileClip`이고 지원되는 확장자를 가진 유효한 파일 이름을 포함하는 경우, Weave는 파일을 직접 업로드합니다.
...
비디오 객체가 메모리에만 존재하고(디스크에 저장된 파일이 없는 경우), Weave는 이를 `.mp4` 파일로 인코딩하고 업로드를 자동으로 처리합니다. 이는 다음 유형의 클립에 적용됩니다:
...
다음 코드 예시는 Weave에서 비디오 처리 함수를 어떻게 트레이싱하는지 보여줍니다. 이 코드 예시는 다음을 수행합니다:

1. Weave 프로젝트 `video-test`를 초기화합니다.
2. 로드된 `VideoFileClip`에서 1초짜리 서브클립을 `VideoClip`으로 추출하는 함수 `get_video`를 `weave.op`으로 추적하도록 정의합니다.
3. 클립을 Weave에 업로드하고 추적합니다.
4. 비디오가 없으면 더미 MP4 비디오를 자동으로 생성합니다.

<Warning>
  스레드 안전성 문제를 피하려면 Weave `op` 외부에서 `VideoFileClip` 객체를 생성하지 말고, 항상 `VideoFileClip`에 대한 경로만 전달해야 합니다.
</Warning>

다음 코드 스니펫을 사용하기 전에 [사용을 위한 사전 요구 사항](#usage-prerequisites)을 먼저 완료하세요.
...
코드 샘플이 성공적으로 실행되면 프로젝트의 **Traces** 테이블에서 링크를 클릭해 비디오를 볼 수 있습니다.

<Frame>
  ![Traces 테이블에 있는 비디오 처리 함수 트레이스.](/weave/guides/tracking/imgs/video-trace.png)
</Frame>

<Frame>
  ![Traces 팝업에서 확인하는 Weave에 업로드된 MP4 비디오.](/weave/guides/tracking/imgs/video-trace-popout.png)
</Frame>
