...
자가 관리형 Weave 배포는 백엔드를 관리하기 위해 [ClickHouseDB](https://clickhouse.com/)에 의존합니다. 이 배포에서는 다음 구성 요소를 사용합니다:
...
- **보안 및 컴플라이언스**: 조직의 보안 정책과 Kubernetes/OpenShift 요구사항에 맞게 security context, `runAsUser`/`fsGroup` 값, 기타 보안 설정을 조정하십시오.
...
- **Storage Class**: 영구 볼륨용으로 정상 동작하는 StorageClass (예: `gp3`, `standard`, `nfs-csi`)
...
<Warning>
이 사전 준비 사항 목록만을 기반으로 리소스 규모를 결정하지 마십시오. 필요한 리소스는 트레이스 양과 사용 패턴에 따라 크게 달라집니다. 구체적인 클러스터 규모 산정 가이드는 상세한 [Resource Requirements](#resource-requirements) 섹션을 참조하십시오.
</Warning>
...
`ch-operator.yaml`이라는 이름의 파일을 만듭니다:
...
여기 표시된 `containerSecurityContext` 값은 대부분의 Kubernetes 배포판에서 잘 작동합니다. **OpenShift**의 경우, 프로젝트에 할당된 UID 범위와 일치하도록 `runAsUser`와 `fsGroup`을 조정해야 할 수도 있습니다.
...
AWS 계정 또는 S3와 호환되는 스토리지 서비스에서 S3 버킷을 생성합니다:
...
S3 액세스 자격 증명을 제공하는 방법은 두 가지입니다:
...
그런 다음 ClickHouse가 해당 시크릿을 사용하도록 구성하세요(아래 ch-server.yaml 구성을 참조하세요).
...
[ClickHouse Keeper](https://clickhouse.com/docs/guides/sre/keeper/clickhouse-keeper)는 데이터 복제 및 분산 DDL 쿼리 실행을 위한 조정 시스템 역할을 합니다.

<div id="31-create-the-keeper-configuration">
  #### 3.1 Keeper 구성 파일 생성
</div>

`ch-keeper.yaml`라는 이름의 파일을 생성합니다.
...
* **StorageClass**: 클러스터에서 사용 가능한 StorageClass에 맞도록 `storageClassName: gp3` 값을 업데이트하십시오.
* **Security Context**: 조직의 보안 정책을 준수하도록 `runAsUser`, `fsGroup` 값을 조정하십시오.
* **Anti-Affinity**: 클러스터 토폴로지와 고가용성(HA) 요구 사항에 따라 `affinity` 섹션을 수정하거나 제거하십시오.
* **Resources**: CPU/메모리 값은 예시입니다. 적절한 사이징을 위해 W&amp;B 솔루션 아키텍트와 상의하십시오.
* **Naming**: `metadata.name` 또는 `configuration.clusters[0].name`을 변경하는 경우, ch-server.yaml(4단계)에서 Keeper 호스트 이름을 일치하도록 **반드시 업데이트해야 합니다**.
...
이제 Weave 추적 데이터를 저장할 ClickHouse 서버 클러스터를 배포합니다.
...
1. **StorageClass**: `storageClassName: gp3`를 클러스터의 StorageClass에 맞게 업데이트합니다.
...
5. **Resource Allocation**: CPU/메모리 값은 예시일 뿐이므로, 예상 트레이스 볼륨에 따른 적절한 사이징을 위해 **W&B 솔루션 아키텍트와 상의**하십시오.
6. **Anti-Affinity Rules**: 클러스터 토폴로지와 고가용성 요구 사항에 따라 규칙을 수정하거나 제거합니다.
7. **Keeper Hostnames**: Keeper 노드 호스트명은 3단계에서 사용한 Keeper 배포 네이밍과 **반드시 일치해야** 합니다(아래 "Keeper 네이밍 이해하기" 참조).
8. **Cluster Naming**: 클러스터 이름 `weavecluster`는 변경할 수 있지만, 5단계의 `WF_CLICKHOUSE_REPLICATED_CLUSTER` 값과 일치해야 합니다.
9. **Credentials**:
   - IRSA의 경우: `<use_environment_credentials>true</use_environment_credentials>`를 유지하거나, 환경 변수에 매핑된 시크릿 키를 통해 액세스합니다.
...
<div id="43-configure-credentials-option-b-only">
  #### 4.3 자격 증명 구성 (옵션 B에만 해당)
</div>

2단계에서 **옵션 B(액세스 키)** 를 사용한 경우, `ch-server.yaml`의 `env` 섹션이 해당 시크릿을 참조하도록 설정되어 있는지 확인합니다.
...
**옵션 A (IRSA)**를 사용하는 경우 전체 `env` 섹션을 제거하십시오.


<div id="44-understanding-keeper-naming">
  #### 4.4 Keeper 명명 방식 이해하기
</div>
...
* `{replica-index}` = 레플리카 번호: 3개 레플리카인 경우 `0`, `1`, `2`
...
<Note>
  `ch-server.yaml`의 Keeper 호스트 이름은 Keeper 배포에서 생성된 실제 서비스 이름과 **정확히 일치해야** 하며, 일치하지 않으면 ClickHouse 서버가 코디네이션 서비스에 연결하지 못합니다.
</Note>
...
이제 W&B 플랫폼이 Weave 트레이스용으로 ClickHouse 클러스터를 사용하도록 구성합니다.
...
* `WF_CLICKHOUSE_REPLICATED_CLUSTER: "weavecluster"` - ch-server.yaml의 클러스터 이름과 **동일해야 함**
...
이를 실행한 후 조직의 traces 페이지에서 W&amp;B UI를 열어 trace가 생성되었는지 확인하십시오.
...
<div id="clickhouse-keeper-issues">
  ### ClickHouse Keeper 관련 문제
</div>

**문제**: Keeper 파드가 `Pending` 상태에서 멈춰 있는 경우
...
`StorageClass`가 올바르게 구성되어 있고 충분한 사용 가능 용량이 있는지 확인하세요.
...
* 더 유연한 안티-어피니티를 위해 `requiredDuringSchedulingIgnoredDuringExecution` 대신 `preferredDuringSchedulingIgnoredDuringExecution`을 사용합니다.
...
연결이 실패하면 ch-server.yaml에 설정된 Keeper 호스트 이름이 실제 Keeper 배포와 일치하지 않을 가능성이 큽니다. 이름 지정 패턴은 4단계의 &quot;Keeper 이름 규칙 이해하기&quot;를 참고하세요.
...
1. 라이선스에 Weave가 포함되어 있는지 확인:

   ```bash
   kubectl get secret license-key -n wandb -o jsonpath='{.data.value}' | base64 -d | jq
   ```

2. wandb-cr.yaml에서 `weave-trace.enabled: true` 및 `clickhouse.replicated: true`가 설정되어 있는지 확인

3. W&amp;B operator 로그를 확인:
   ```bash
...
**항상 W&B 솔루션 아키텍트 팀과 상의하여** 귀하의 구체적인 사용 사례에 맞는 적절한 규모를 결정하십시오. 리소스를 과소 할당하면 성능 문제가 발생할 수 있고, 과대 할당하면 인프라 비용이 낭비됩니다.
</Warning>
...
| 구성 요소         | 레플리카 수 | CPU 요청 / 제한     | 메모리 요청 / 제한     | 스토리지  |
| ----------------- | ----------- | ------------------- | ---------------------- | --------- |
...
_적용 대상: 개발, 테스트, 저 트래픽 프로덕션 환경_

<div id="recommended-production-setup">
  ### 권장 프로덕션 설정
</div>

트레이스량이 많은 프로덕션 워크로드의 경우:
...
_적합한 환경: 대량 트레이스 프로덕션 환경_

초고용량 트레이스 배포의 경우, 구체적인 트레이스량과 성능 요구 사항에 따라 맞춤형 사이징 권장 사항을 받으려면 W&B 솔루션 아키텍트 팀에 문의하십시오.

<div id="advanced-configuration">
  ## 고급 구성
</div>

이 섹션에서는 자체 관리형 Weave 배포에서 사용할 수 있는 사용자 지정 옵션을 다룹니다. 여기에는 ClickHouse 용량을 수직 확장 또는 수평 확장을 통해 늘리는 방법, keeper 및 server 구성에서 이미지 태그를 수정하여 ClickHouse 버전을 업데이트하는 방법, 그리고 ClickHouse 상태를 모니터링하는 방법이 포함됩니다.

인스턴스에 고급 수준의 변경을 적용할 때는 성능 및 안정성 요구 사항을 충족하는지 확인하기 위해 W&B 솔루션 아키텍트 팀과 상의하시길 권장합니다.

<div id="scaling-clickhouse">
  ### ClickHouse 확장
</div>

ClickHouse의 용량을 늘리려면 다음을 수행할 수 있습니다:

1. **수직 확장(Vertical Scaling)**: 파드당 리소스를 늘립니다(더 단순한 접근 방식).
   ...
   **권장 사항**: 실제 리소스 사용량을 모니터링하고 그에 따라 확장하십시오. 초고용량 배포의 경우 W&B Solutions Architect 팀에 문의하십시오.
...
호환성을 위해 Keeper 버전과 서버 버전은 일치하거나 Keeper 버전이 서버 버전보다 크거나 같아야 (&gt;=) 합니다.
...
모니터링을 위해 ClickHouse 시스템 테이블에 접근하세요:
...
ClickHouse 데이터는 S3에 저장되어 S3 버전 관리와 버킷 복제 기능을 통해 고유의 백업 기능을 제공합니다. 배포 환경별로 적합한 백업 전략을 수립하려면 W&B 솔루션 아키텍트 팀과 상의하고 [ClickHouse 백업 문서](https://clickhouse.com/docs/en/operations/backup)를 참조하세요.
...
2. **네트워크 정책**: ClickHouse 액세스를 제한하기 위해 Kubernetes NetworkPolicy 리소스 적용을 고려하세요
3. **RBAC**: ServiceAccount에 최소 필요 권한만 부여했는지 확인하세요
4. **S3 버킷**: 저장 시 암호화를 활성화하고, 필요한 IAM 역할에만 버킷 액세스를 허용하세요
...
`ch-server.yaml`에서 이미지 버전을 업데이트한 뒤 적용합니다:
...
`wandb-cr.yaml`에서 이미지 태그를 업데이트한 후 적용합니다:
...
A: 예, 가능하지만 프로덕션 환경에서는 권장하지 않습니다. ch-server.yaml에서 `replicasCount: 1`로 수정하고 wandb-cr.yaml에서 `clickhouse.replicated: false`로 설정하세요.
...
**Q: 제 클러스터 이름이 `weavecluster`가 아니라면 어떻게 하나요?**
