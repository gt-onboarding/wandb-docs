...
description: "weave.trace_server_bindings.remote_http_trace_server에 대한 Python SDK 참조 문서"
---
...
내부 호출 프로세서를 노출하기 위해 공식 `TraceServerInterface`에 정의되지 않은 커스텀 메서드입니다. 클라이언트 측 인터페이스에 정식으로 포함되어야 합니다.
...
내부 피드백 프로세서를 노출하기 위한 용도로, 공식 TraceServerInterface에 정의되어 있지 않은 커스텀 메서드입니다. 클라이언트 측 인터페이스에 정식으로 포함되어야 합니다.
...
`calls/batch_upsert`와 마찬가지로, 테이블 업데이트를 일련의 업데이트로 분해할 수 있는 성질을 이용해 페이로드 크기를 동적으로 조정할 수 있습니다.
