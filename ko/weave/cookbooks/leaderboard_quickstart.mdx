---
title: "Leaderboard 빠른 시작"
description: "W&B Weave에서 Leaderboard 빠른 시작을 사용하는 방법을 알아봅니다"
---

<Note>
이 노트북은 대화형 노트북입니다. 로컬에서 실행하거나 아래 링크를 이용하세요:
...
이 노트북에서는 Weave의 리더보드를 사용하여 서로 다른 데이터셋과 스코어링 함수 전반에서 모델 성능을 비교하는 방법을 알아봅니다. 구체적으로 다음을 수행합니다:

1. 가짜 우편번호 데이터로 구성된 데이터셋을 생성합니다.
2. 몇 가지 스코어링 함수를 정의하고 기준선 모델을 평가합니다.
3. 이러한 기법을 사용해 여러 모델과 여러 평가의 조합으로 이루어진 행렬을 평가합니다.
4. Weave UI에서 리더보드를 확인합니다.

<div id="step-1-generate-a-dataset-of-fake-zip-code-data">
  ## 1단계: 가상의 우편번호 데이터셋 생성
</div>

먼저 가상의 우편번호 데이터셋을 생성하는 함수 `generate_dataset_rows`를 정의합니다.
...
이제 다음 3개의 스코어링 함수를 작성합니다:

1. `check_concrete_fields`: 모델 출력이 예상된 도시(city)와 주(state)와 일치하는지 확인합니다.
2. `check_value_fields`: 모델 출력이 예상된 인구(population)와 중간 소득(median income)의 10% 이내인지 확인합니다.
3. `check_subjective_fields`: LLM을 사용하여 모델 출력이 예상된 &quot;known for&quot; 필드와 일치하는지 확인합니다.
...
이제 가짜 데이터와 점수 함수들을 사용하여 간단한 평가를 정의합니다.
...
이제 항상 동일한 응답을 반환하는 베이스라인 모델을 평가해 보겠습니다.
...
이제 베이스라인과 비교할 수 있도록 모델 2개를 더 만듭니다.
...
이제 모델과 평가의 조합으로 이루어진 매트릭스를 평가해 보겠습니다.
...
UI에서 리더보드 탭으로 이동한 다음 &quot;Create Leaderboard&quot;를 클릭해 새 리더보드를 만들 수 있습니다.
