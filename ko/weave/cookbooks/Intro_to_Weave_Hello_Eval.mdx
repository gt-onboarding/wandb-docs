---
title: "평가 소개"
description: "W&B Weave로 평가를 사용하는 방법을 알아보세요"
---
...
Weave 평가를 실행하기 전에 다음을 완료하세요.

1. W&amp;B Weave SDK를 설치하고 [API key](https://wandb.ai/settings#apikeys)로 로그인합니다.
2. OpenAI SDK를 설치하고 [API key](https://platform.openai.com/api-keys)로 로그인합니다.
3. W&amp;B 프로젝트를 초기화합니다.
...
# 🔑 API key 설정
# 이 셀을 실행하면 `getpass`를 통해 API key 입력을 요청하며, 입력 내용은 터미널에 표시되지 않습니다.
#####
print("---")
print(
...
다음 코드 예시는 Weave의 `Model` 및 `Evaluation` API를 사용해 LLM을 평가하는 방법을 보여줍니다. 먼저 `weave.Model`을 상속해 Weave 모델을 정의하고, 모델 이름과 프롬프트 형식을 지정한 뒤, `@weave.op`으로 `predict` 메서드를 추적합니다. `predict` 메서드는 프롬프트를 OpenAI로 전송하고, Pydantic 스키마(`FruitExtract`)를 사용해 응답을 구조화된 출력으로 파싱합니다. 그런 다음 입력 문장과 기대하는 타깃으로 구성된 작은 평가 데이터셋을 만듭니다. 다음으로, 모델의 출력을 타깃 레이블과 비교하는 사용자 정의 스코어링 함수(역시 `@weave.op`으로 추적)를 정의합니다. 마지막으로, 데이터셋과 스코어러를 지정해 이 모든 것을 `weave.Evaluation`으로 감싼 다음, `evaluate()`를 호출해 평가 파이프라인을 비동기 방식으로 실행합니다.
...
* [RAG 애플리케이션을 구축하며 평가하기](/ko/weave/tutorial-rag)에 대해 알아보세요.