...
환경 변수에 `WANDB_API_KEY`를 설정하여 `wandb.login()`으로 쉽게 로그인할 수 있게 합니다(이 값은 Colab에 secret으로 제공되어야 합니다).

W&amp;B에서 로그를 남길 프로젝트를 `name_of_wandb_project`에 설정합니다.
...
여기서는 사용자 정의 비용을 추가해 보겠습니다. 이제 사용자 정의 비용이 있고 호출에 사용량 정보도 있으므로, `include_cost`를 사용해 호출을 가져올 수 있으며, 각 호출의 비용은 `summary.weave.costs` 아래에 포함됩니다.
