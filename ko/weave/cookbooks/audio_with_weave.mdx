...
고급 예제에서는 OpenAI Realtime API를 활용하여 오디오를 실시간으로 스트리밍합니다. 아래 썸네일을 클릭하여 동영상 데모를 보거나, [여기](https://www.youtube.com/watch?v=lnnd73xDElw)를 클릭하세요.
...
먼저 OpenAI(`openai`)와 Weave(`weave`) 패키지, 그리고 API key 관리를 위한 `set-env` 패키지를 설치합니다.
...
다음으로 OpenAI와 Weave에 필요한 API key를 불러옵니다. 여기서는 Google Colab의 비밀 키 관리자와 호환되는 set&#95;env를 사용하며, Colab 전용 `google.colab.userdata`의 대안으로 활용할 수 있습니다. 사용 방법은 [여기](https://pypi.org/project/set-env-colab-kaggle-dotenv/)를 참고하세요.
...
이제 오디오 모달리티를 활성화한 상태로 OpenAI의 completions 엔드포인트를 호출하도록 설정합니다. 먼저 OpenAI 클라이언트를 생성하고 Weave Project를 초기화하세요.
...
이제 OpenAI completions 요청을 정의하고 Weave 데코레이터(op)를 추가합니다.

여기서는 함수 `prompt_endpont_and_log_trace`를 정의합니다. 이 함수는 세 가지 주요 단계를 가집니다:

1. 텍스트와 오디오 입력 및 출력을 모두 지원하는 `GPT 4o Audio Preview` 모델을 사용하여 completion 객체를 만듭니다.

   * 모델에게 다양한 억양으로 천천히 13까지 세도록 프롬프트를 줍니다.
   * completion을 &quot;stream&quot;으로 설정합니다.

2. 스트리밍된 데이터가 청크 단위로 쓰이도록 새 출력 파일을 엽니다.

3. 오디오 파일에 대한 열린 파일 핸들을 반환하여 Weave가 트레이스에서 오디오 데이터를 기록할 수 있도록 합니다.
...
다음 셀을 실행하세요. 시스템 프롬프트와 사용자 프롬프트, 그리고 출력 오디오가 Weave trace에 함께 저장됩니다.
셀을 실행한 후, &quot;🍩&quot; 이모지 옆에 있는 링크를 클릭해 trace를 확인하세요.
...
OpenAI의 realtime API는 실시간 오디오 및 텍스트 어시스턴트를 구축하기 위한, 기능이 풍부하고 신뢰성이 높은 대화형 API입니다.

다음 사항을 참고하세요:

- [Microphone Configuration](#microphone-configuration)의 셀을 검토하세요.
- Google Colab 실행 환경의 한계로 인해, **반드시 호스트 머신에서 Jupyter Notebook으로 실행해야 합니다.** 브라우저 내에서는 실행할 수 없습니다.
  - MacOS에서는 Pyaudio가 동작하도록 Brew를 통해 `portaudio`를 설치해야 합니다(자세한 내용은 [여기](https://formulae.brew.sh/formula/portaudio)를 참고하세요).
- OpenAI의 Python SDK는 아직 Realtime API를 지원하지 않습니다. 더 높은 가독성을 위해 Pydantic으로 전체 OAI Realtime API 스키마를 구현했으며, 공식 지원이 출시되면 이 구현은 더 이상 사용되지 않을 수 있습니다.
- `enable_audio_playback` 토글을 활성화하면 어시스턴트가 출력한 오디오가 재생됩니다. **이 기능을 활성화하는 경우 반드시 헤드폰을 사용해야 합니다.** 에코 감지를 제대로 구현하려면 매우 복잡한 처리가 필요하기 때문입니다.
...
다음 셀을 실행해 사용 가능한 모든 오디오 장치를 확인하세요. 그런 다음 표시된 장치 목록을 기준으로 `INPUT_DEVICE_INDEX`와 `OUTPUT_DEVICE_INDEX` 값을 설정하세요. 입력 장치는 최소 1개 이상의 입력 채널이, 출력 장치는 최소 1개 이상의 출력 채널이 있어야 합니다.
...
python
INPUT_DEVICE_INDEX = 3  # @param                                                 # 위의 장치 목록을 기반으로 선택하세요. 장치의 입력 채널 수가 0보다 많은지 확인하세요.
OUTPUT_DEVICE_INDEX = 12  # @param                                                # 위의 장치 목록을 기반으로 선택하세요. 장치의 출력 채널 수가 0보다 많은지 확인하세요.
...
OpenAI Python SDK는 아직 Realtime API를 지원하지 않습니다. 가독성을 높이기 위해 전체 OAI Realtime API 스키마를 Pydantic으로 구현했으며, 공식 지원이 추가되면 이 구현은 사용 중단(deprecate)될 수 있습니다.

<div id="pydantic-schema-for-openai-realtime-api">
  ### OpenAI Realtime API를 위한 Pydantic 스키마
</div>
...
class BaseEvent(BaseModel):
    ...
    event_id: Optional[str] = None  # 모든 이벤트에 대한 선택적 필드로 event_id 추가
...
# 모든 이벤트 타입을 포함하도록 Event 유니온 업데이트
ClientEvent = Union[
    SessionUpdate,
...
realtime (RT) audio 모델은 websocket을 사용해 이벤트를 OpenAI의 Realtime audio API로 전송합니다. 동작 방식은 다음과 같습니다:

1.  **init:** 로컬 버퍼(입력 오디오)와 스트림(assistant 재생 스트림, 사용자 오디오 디스크 기록 스트림)을 초기화하고 Realtime API에 대한 연결을 엽니다.
2.  **receive_messages_thread**: 별도 스레드가 API에서 오는 메시지 수신을 처리합니다. 네 가지 주요 이벤트 타입을 처리합니다: - RESPONSE_AUDIO_TRANSCRIPT_DONE:

            서버가 assistant의 응답이 완료되었음을 알리고 transcript를 제공합니다.

        - CONVERSATION_ITEM_INPUT_AUDIO_TRANSCRIPTION_COMPLETED:

            서버가 사용자의 오디오가 텍스트로 변환되었음을 알리고, 사용자의 오디오 transcript를 전송합니다. 여기서는 transcript를 Weave에 로그로 남기고, 사용자에게 출력합니다.

        - RESPONSE_AUDIO_DELTA:

            서버가 assistant 응답 오디오의 새로운 청크(chunk)를 전송합니다. 이를 response ID를 기준으로 진행 중인 응답 데이터에 추가하고, 재생을 위해 출력 스트림에 추가합니다.

        - RESPONSE_DONE:

            서버가 assistant 응답이 완료되었음을 알립니다. 이 응답과 연관된 모든 오디오 청크와 transcript를 가져와 Weave에 기록합니다.

    3. **send_audio**: 핸들러가 사용자 오디오 청크를 버퍼에 추가하고, 오디오 버퍼가 특정 크기에 도달했을 때 오디오 청크를 전송합니다.

```python lines
class RTAudioModel(weave.Model):
    """로깅을 위한 Whisper 사용자 전사 기능을 포함한 실시간 e2e 오디오 OpenAI 모델 상호작용을 위한 Model 클래스."""
    ...
    def handle_assistant_response_done(self, data: ResponseDone):
        ...
        weave_payload = {
            ...
            .content[0]
            .get("transcript", "전사 내용을 사용할 수 없습니다."),
        }
    ...
    def send_audio(self, audio_chunk):
        if self.ws and self.ws.connected:
            ...
            print("오디오 전송 오류: websocket이 초기화되지 않았습니다.")
...
`RTAudio` 모델의 `send_audio` 메서드에 연결된 핸들러를 갖는 PyAudio 입력 스트림을 사용합니다. 이 스트림은 프로그램이 종료될 때 안전하게 종료할 수 있도록 메인 스레드로 반환됩니다.
...
메인 스레드는 Weave가 통합된 실시간 오디오 모델을 초기화합니다. 이어서 녹음을 시작한 뒤, 사용자로부터 키보드 인터럽트가 발생할 때까지 대기합니다.
