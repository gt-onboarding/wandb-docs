...
<div id="using-huggingface-datasets-in-evaluations-with-preprocess_model_input">
  # 평가에서 `preprocess_model_input`과 함께 Hugging Face Datasets 사용하기
</div>
...
> 이 가이드는 Weave 평가에서 HuggingFace Datasets를 사용하기 위한 임시 우회 방법을 보여줍니다.<br /><br/>
...
먼저 실험을 추적할 수 있도록 Weave를 초기화하고 Weights &amp; Biases에 연결합니다.
...
> 이 고유한 다이제스트 값은 평가 시 특정 데이터셋 항목을 추적하고 참조하는 데 사용됩니다.
...
* `hf_eval`: 모델 출력에 대한 점수 계산 방식을 정의합니다
* `function_to_evaluate`: 평가할 실제 함수/모델입니다
