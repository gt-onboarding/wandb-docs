...
이 가이드에서는 W&amp;B Weave를 사용할 때 개인식별정보(PII, Personally Identifiable Information) 데이터의 프라이버시를 보호하는 방법을 설명합니다. 이 가이드는 PII 데이터를 식별, 마스킹(redact), 익명화(anonymize)하는 다음 방법들을 다룹니다:

1. PII 데이터를 식별하고 마스킹하기 위한 **정규식(regular expressions)**.
2. 파이썬 기반 데이터 보호 SDK인 **Microsoft의 [Presidio](https://microsoft.github.io/presidio/)**. 이 도구는 마스킹 및 대체(replacement) 기능을 제공합니다.
3. PII 데이터를 익명화하기 위해 Presidio와 함께 사용하는, 가짜 데이터를 생성하는 Python 라이브러리 **[Faker](https://faker.readthedocs.io/en/master/)**.

또한 *`weave.op` 입력/출력 로깅 커스터마이징* 및 &#95;`autopatch_settings`&#95;을 사용하여 PII 마스킹 및 익명화를 워크플로에 통합하는 방법을 설명합니다. 자세한 내용은 [로그되는 입력과 출력 커스터마이즈](/ko/weave/guides/tracking/ops/#customize-logged-inputs-and-outputs)를 참고하세요.

시작하려면 다음 단계를 따르세요:

1. [개요](#overview) 섹션을 확인합니다.
...
다음 섹션에서는 `weave.op`을 사용한 입력 및 출력 로깅 방식과, Weave에서 PII 데이터를 처리할 때의 모범 사례를 살펴봅니다.

<div id="customize-input-and-output-logging-using-weaveop">
  ### `weave.op`을 사용해 입력 및 출력 로깅 사용자 정의하기
</div>
...
PII 데이터와 함께 Weave를 사용하기 전에, 해당 모범 사례를 먼저 검토하십시오.
...
PII 데이터를 탐지하고 보호하기 위해 아래 방법들을 사용해 PII 데이터를 식별·비식별화하고, 필요에 따라 익명화합니다:

1. **정규식(regular expressions)** 으로 PII 데이터를 식별하고 비식별화합니다.
2. **Microsoft [Presidio](https://microsoft.github.io/presidio/)**: 비식별화 및 치환 기능을 제공하는 Python 기반 데이터 보호 SDK입니다.
...
[정규 표현식(regular expressions, regex)](https://docs.python.org/3/library/re.html)은 PII 데이터를 식별하고 비식별 처리(마스킹)하는 가장 간단한 방법입니다. Regex를 사용하면 전화번호, 이메일 주소, 사회보장번호와 같은 민감한 정보의 다양한 형식에 일치하는 패턴을 정의할 수 있습니다. Regex를 이용해 방대한 양의 텍스트를 검사하면서 더 복잡한 NLP 기법 없이도 정보를 대체하거나 비식별 처리할 수 있습니다.
...
다음 방법은 [Microsoft Presidio](https://microsoft.github.io/presidio/)를 사용해 PII 데이터를 완전히 제거(비식별화)하는 방식입니다. Presidio는 PII를 마스킹하고 PII 유형을 나타내는 플레이스홀더로 대체합니다. 예를 들어 `"My name is Alex"`에서 `Alex`를 `<PERSON>`으로 대체합니다.

Presidio에는 [일반적인 엔티티](https://microsoft.github.io/presidio/supported_entities/)에 대한 기본 지원이 포함되어 있습니다. 아래 예제에서는 `PHONE_NUMBER`, `PERSON`, `LOCATION`, `EMAIL_ADDRESS`, `US_SSN`인 모든 엔티티를 마스킹합니다. Presidio 처리 과정은 하나의 함수로 캡슐화되어 있습니다.
...
텍스트를 마스킹하는 대신, MS Presidio를 사용해 이름, 전화번호와 같은 PII를 [Faker](https://faker.readthedocs.io/en/master/) Python 라이브러리가 생성한 가짜 데이터로 치환하여 익명화할 수 있습니다. 예를 들어, 다음과 같은 데이터가 있다고 가정해 보겠습니다:
...
코드를 하나의 클래스로 통합하고, 앞에서 확인한 추가 엔티티들을 포함하도록 엔티티 목록을 확장해 보겠습니다.
...
`autopatch_settings`를 사용하면 지원되는 하나 이상의 LLM 통합에 대해 초기화 시점에 PII 처리를 직접 구성할 수 있습니다. 이 방법의 장점은 다음과 같습니다.

1. PII 처리 로직이 초기화 시점에 중앙집중식으로 정의되므로, 여기저기 흩어진 커스텀 로직을 둘 필요가 줄어듭니다.
2. 특정 통합에 대해서는 PII 처리 워크플로를 커스터마이즈하거나 완전히 비활성화할 수 있습니다.

`autopatch_settings`를 사용해 PII 처리를 구성하려면, 지원되는 LLM 통합 중 하나의 `op_settings`에서 `postprocess_inputs` 및/또는 `postprocess_output`을 정의하십시오.
...
다음 예제에서는 PII 마스킹 및 익명화 메서드를 Weave Models에 통합하고, 결과를 Weave Traces에서 미리 확인합니다.

먼저 [Weave Model](https://docs.wandb.ai/weave/guides/core-types/models)을 생성합니다. Weave Model은 구성 설정, 모델 가중치, 코드처럼 모델 동작 방식을 정의하는 정보의 조합입니다.

이 모델 안에 Anthropic API를 호출하는 `predict` 함수를 포함합니다. Anthropic의 Claude Sonnet은 [Traces](https://docs.wandb.ai/weave/quickstart)를 사용해 LLM 호출을 추적하면서 감성 분석을 수행하는 데 사용됩니다. Claude Sonnet은 텍스트 블록을 입력으로 받아 다음 감성 분류 중 하나를 출력합니다: *positive*, *negative*, 또는 *neutral*. 추가로, PII 데이터가 LLM으로 전송되기 전에 마스킹되거나 익명화되도록 후처리(postprocessing) 함수도 포함합니다.
...
이제 Presidio를 사용해 원본 텍스트에서 PII 데이터를 식별하고 마스킹합니다.

<Frame>
  ![식별된 PII 엔터티와 마스킹된 텍스트 출력이 표시된 Presidio PII 마스킹 과정](/media/pii/redact.png)
</Frame>
...
이 예제에서는 Faker를 사용해 익명화된 대체 PII 데이터를 생성하고, Presidio를 사용해 원본 텍스트에서 PII 데이터를 식별한 뒤 해당 데이터를 대체합니다.

<Frame>
  ![원본 텍스트, 식별된 PII, 익명화된 대체 값이 포함된 Faker 및 Presidio PII 치환 프로세스](/media/pii/replace.png)
</Frame>
...
다음 예제에서는 초기화 시 `anthropic`에 대한 `postprocess_inputs`를 `postprocess_inputs_regex()` 함수로 설정합니다. `postprocess_inputs_regex` 함수는 [Method 1: Regular Expression Filtering](#method-1-regular-expression-filtering)에서 정의한 `redact_with_regex` 메서드를 적용합니다. 이제 `redact_with_regex`가 모든 `anthropic` 모델의 모든 입력에 자동으로 적용됩니다.
...
<Frame>
  ![암호화된 텍스트 출력과 암호화 키 관리를 포함한 PII 데이터 암호화 프로세스](/media/pii/encrypt.png)
</Frame>

PII를 익명화하는 것과 함께, `cryptography` 라이브러리의 [Fernet](https://cryptography.io/en/latest/fernet/) 대칭키 암호화를 사용해 데이터에 추가적인 보안 계층을 더할 수 있습니다. 이 방식은 익명화된 데이터가 가로채이더라도 암호화 키 없이는 내용을 해독할 수 없도록 보장합니다.
