...
[Track LLM inputs &amp; outputs](/ko/weave/quickstart) 튜토리얼에서는 LLM의 입력과 출력을 추적하는 기본 개념을 다루었습니다.
...
[퀵스타트 예제](/ko/weave/quickstart)를 기반으로, 다음 코드는 LLM에서 반환된 항목 수를 세는 추가 로직을 더하고, 이를 상위 수준 함수로 모두 감쌉니다. 또한 이 예제는 `weave.op()`을 사용하여 모든 함수, 그 호출 순서, 그리고 부모-자식 관계를 추적합니다:
...
<Tabs>
  <Tab title="Python">
    ...
    **중첩 함수**
    ...
    위 코드를 실행하면 두 개의 중첩 함수(`extract_dinos`와 `count_dinos`)의 입력과 출력뿐 아니라 자동으로 로깅된 OpenAI 트레이스도 확인할 수 있습니다.

    ![중첩 Weave 트레이스](/images/tutorial_tracing_2_nested_dinos.png)
  </Tab>
  ...
  <Tab title="TypeScript">
    ...
    **중첩 함수**
    ...
    위 코드를 실행하면 두 개의 중첩 함수(`extractDinos`와 `countDinos`)의 입력과 출력뿐 아니라 자동으로 로깅된 OpenAI 트레이스도 확인할 수 있습니다.
...
<Note>
  실행 시점에 사용자 ID, 코드의 환경 상태(개발, 스테이징, 프로덕션 등)와 같은 메타데이터를 추적할 것을 권장합니다.
...
* [App Versioning 튜토리얼](/ko/weave/tutorial-weave_models)을(를) 따라 임시 프롬프트, 모델, 애플리케이션 변경 사항을 기록하고 버전 관리하며 체계적으로 정리하세요.