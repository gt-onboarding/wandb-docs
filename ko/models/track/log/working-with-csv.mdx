---
description: W&B로 데이터 가져오기 및 로깅
title: 실험과 함께 CSV 파일 추적하기
---

W&amp;B Python 라이브러리를 사용해 CSV 파일을 로깅하고 [W&amp;B Dashboard](/ko/models/track/workspaces/)에서 시각화할 수 있습니다. W&amp;B Dashboard는 머신러닝 모델 결과를 구성하고 시각화하는 중앙 허브입니다. 이는 W&amp;B에 로깅되지 않은 [이전 머신러닝 실험 정보가 담긴 CSV 파일](#import-and-log-your-csv-of-experiments)이 있거나, [데이터셋이 담긴 CSV 파일](#import-and-log-your-dataset-csv-file)이 있는 경우 특히 유용합니다.
...
1. 시작하려면 먼저 CSV 파일을 가져옵니다. 다음 코드 스니펫에서 `iris.csv` 파일 이름을 사용 중인 CSV 파일 이름으로 바꾸세요:
...
3. 다음으로 W&amp;B 아티팩트를 생성하고 해당 아티팩트에 테이블을 추가합니다.
...
W&amp;B 아티팩트에 대한 자세한 내용은 [Artifacts 장](/ko/models/artifacts/)을 참조하세요.

4. 마지막으로, 새로운 W&amp;B 실행을 시작하고 `wandb.init`으로 W&amp;B에 추적 및 로그를 기록하세요:
...
{/* {% embed url="https://drive.google.com/file/d/1PL4RSdopHEptDR5Gi0DEzECXuoW_5B0f/view?usp=sharing" %}
  아래 표는 변환 후 W&B 대시보드에서 다음과 같이 표시됩니다
  {% endembed %} */}
...
* 실험에 필요한 설정(추가로 [Sweeps Hyperparameter Tuning](/ko/models/sweeps/)을 활용할 수 있는 장점 포함)
...
1. 시작하려면 먼저 CSV 파일을 읽어 Pandas DataFrame으로 변환합니다. `"experiments.csv"`를 사용자의 CSV 파일 이름으로 바꾸십시오:
...
2. 다음으로, 새 W&amp;B 실행을 시작해 [`wandb.init()`](/ko/models/ref/python/functions/init)으로 W&amp;B에 추적하고 로그를 기록하세요:
...
실행이 진행되는 동안, 지표의 각 값을 모두 로깅해 두면 W&amp;B에서 조회, 쿼리, 분석에 활용할 수 있습니다. 이를 위해 [`run.log()`](/ko/models/ref/python/experiments/run/#method-runlog) 명령을 사용하세요:

```python
run.log({key: val})
```

[`define_metric`](/ko/models/ref/python/experiments/run#define_metric) API를 사용해 실행의 결과를 정의하는 최종 요약 지표를 필요에 따라 로깅할 수 있습니다. 다음 예제에서는 `run.summary.update()`를 사용해 실행에 요약 지표를 추가합니다:

```python
run.summary.update(summaries)
```

요약 메트릭에 대한 자세한 내용은 [Log Summary Metrics](./log-summary) 문서를 참조하세요.

아래는 위의 샘플 테이블을 [W&amp;B Dashboard](/ko/models/track/workspaces/)로 변환하는 전체 예제 스크립트입니다:
