...
`wandb` 라이브러리에는 [LightGBM](https://lightgbm.readthedocs.io/en/latest/)을 위한 전용 콜백이 포함되어 있습니다. W&amp;B의 일반 로깅 기능을 사용해 하이퍼파라미터 스윕과 같은 대규모 실험을 추적하는 것도 간편합니다.
...
<Note>
  실행 가능한 코드 예제가 필요하신가요? [GitHub 예제 리포지토리](https://github.com/wandb/examples/tree/master/examples/boosting-algorithms)를 확인하세요.
</Note>
...
모델의 성능을 최대화하려면 트리 깊이, 학습률(learning rate) 같은 하이퍼파라미터를 튜닝해야 합니다. W&amp;B [Sweeps](/ko/models/sweeps/)는 대규모 하이퍼파라미터 탐색 실험을 설정, 오케스트레이션하고 분석할 수 있는 강력한 도구 모음입니다.

이 도구에 대해 더 자세히 알아보고 XGBoost와 함께 Sweeps를 사용하는 예제를 보려면, 이 대화형 Colab 노트북을 살펴보세요.
