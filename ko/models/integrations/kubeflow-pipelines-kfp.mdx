---
description: W&B를 Kubeflow Pipelines와 연동하는 방법
title: Kubeflow Pipelines (kfp)
---

import ApiKeyCreateStreamlined from "/snippets/ko/_includes/api-key-create-streamlined.mdx";

[Kubeflow Pipelines (kfp) ](https://www.kubeflow.org/docs/components/pipelines/overview/)는 Docker 컨테이너를 기반으로 이식성과 확장성을 갖춘 머신러닝(ML) 워크플로우를 구축하고 배포하기 위한 플랫폼입니다.

이 통합을 사용하면 kfp Python 함수형 컴포넌트에 데코레이터를 적용해 매개변수와 아티팩트를 W&amp;B에 자동으로 로깅할 수 있습니다.
...
로컬 환경에 `wandb` 라이브러리를 설치하고 로그인하려면 다음 단계를 수행하세요:
...
컨테이너에 [환경 변수](/ko/models/track/environment-variables/)를 명시적으로 전달해야 할 수 있습니다. 양방향 연결을 사용하려면 환경 변수 `WANDB_KUBEFLOW_URL`에 Kubeflow Pipelines 인스턴스의 기본 URL을 설정해야 합니다. 예를 들어 `https://kubeflow.mysite.com`처럼 설정합니다.
...
Kubeflow Pipelines UI에서 W&amp;B로 로깅된 실행 중 하나를 클릭합니다.

* `Input/Output` 및 `ML Metadata` 탭에서 입력과 출력에 대한 세부 정보를 확인합니다.
* `Visualizations` 탭에서 W&amp;B 웹 앱을 확인합니다.
...
웹 앱 UI는 Kubeflow Pipelines의 `Visualizations` 탭과 동일한 내용을 보여주지만, 더 넓은 화면 공간을 제공합니다. [웹 앱 UI에 대해 자세히 알아보려면 여기를 참조하세요](/ko/models/app).
...
다음은 Kubeflow Pipelines의 개념을 W&amp;B에 매핑한 표입니다.
...
로깅을 더 세밀하게 제어하려면 컴포넌트 내부 곳곳에 `wandb.log`와 `wandb.log_artifact` 호출을 추가하면 됩니다.

<div id="with-explicit-wandblog_artifacts-calls">
  ### 명시적으로 `wandb.log_artifacts` 호출하기
</div>

아래 예제에서는 모델을 학습합니다. `@wandb_log` 데코레이터는 관련 입력과 출력을 자동으로 추적합니다. 학습 과정을 로깅하고 싶다면, 다음과 같이 명시적으로 로깅을 추가할 수 있습니다:
...
[지원되는 프레임워크 통합](/ko/models/integrations)을 사용 중이라면 콜백을 직접 전달할 수도 있습니다.
