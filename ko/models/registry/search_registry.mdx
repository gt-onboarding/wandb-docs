---
title: 레지스트리 항목 찾기
description: "W&B Registry에서 전역 검색창이나 쿼리를 사용하여 레지스트리, 컬렉션, 아티팩트 버전을 검색하는 방법을 알아봅니다."
---

[W&amp;B Registry의 전역 검색창](./search_registry#search-for-registry-items)을 사용하여 레지스트리, 컬렉션, 아티팩트 버전 태그, 컬렉션 태그 또는 별칭을 찾을 수 있습니다. W&amp;B Python SDK를 사용하여 특정 기준에 따라 [레지스트리, 컬렉션, 아티팩트 버전을 필터링](/ko/models/registry/search_registry#query-registry-items)하는 쿼리를 사용할 수 있습니다.
...
2. 페이지 상단의 검색창에 검색어를 입력한 후 Enter 키를 눌러 검색합니다.

입력한 검색어와 일치하는 레지스트리, 컬렉션 이름, 아티팩트 버전 태그, 컬렉션 태그 또는 별칭이 있으면 검색 결과가 검색창 아래에 표시됩니다.

<Frame>
  <img src="/images/registry/search_registry.gif" alt="레지스트리에서 검색하기" />
</Frame>
...
[`wandb.Api().registries()`](/ko/models/ref/python/public-api/api#registries)와 *쿼리 프레디킷(query predicate)*을 사용하여 레지스트리, 컬렉션, 아티팩트 버전을 필터링할 수 있습니다. 쿼리 프레디킷은 반환되는 항목이 충족해야 하는 기준을 지정하는 조건입니다.
...
다음 섹션에서는 사용 가능한 레지스트리 [쿼리 이름](/ko/models/registry/search_registry#filterable-fields), [지원 연산자](/ko/models/registry/search_registry#supported-operators), 그리고 [예시 쿼리](/ko/models/registry/search_registry#example-queries)를 설명합니다.
...
| `$not` | 하나의 조건에 [NOT](https://en.wikipedia.org/wiki/Negation) 논리를 적용합니다 |
...
`wandb.Api().registries()` 메서드를 사용하려면 먼저 W&amp;B Python SDK ([`wandb`](/ko/models/ref/python/)) 라이브러리를 import 하십시오:
...
`model`이라는 문자열이 포함된 모든 레지스트리를 필터링합니다:
...
레지스트리와 관계없이 컬렉션 이름에 `yolo`라는 문자열이 포함된 모든 컬렉션을 필터링합니다:
...
레지스트리와 무관하게, 컬렉션 이름에 문자열 `yolo`가 포함되어 있고 `cnn`을 태그로 가진 모든 컬렉션을 필터링합니다:
...
`model` 문자열을 포함하고 태그가 `image-classification`이거나 `latest` 별칭이 있는 모든 아티팩트 버전을 찾습니다:
...
아티팩트 객체의 전체 속성 목록은 API Reference 문서의 [Artifacts Class](/ko/models/ref/python/experiments/artifact/)를 참고하십시오.
