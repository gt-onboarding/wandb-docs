...
서로 관련된 아티팩트 집합을 나타내는 아티팩트 컬렉션입니다. 

**Args:**

- `client`:  W&B에 쿼리를 보낼 때 사용할 클라이언트 인스턴스. 
 - `entity`:  프로젝트를 소유한 엔터티(사용자 또는 팀). 
 - `project`:  아티팩트 컬렉션을 조회할 프로젝트 이름. 
 - `name`:  아티팩트 컬렉션의 이름. 
 - `type`:  아티팩트 컬렉션의 타입(예: "dataset", "model"). 
 - `organization`:  (해당하는 경우) 선택적인 organization 이름. 
 - `attrs`:  아티팩트 컬렉션을 초기화하기 위한 속성 매핑(선택 사항). 제공되지 않은 경우, 객체는 초기화 시점에 W&B에서 해당 속성을 로드합니다. 
...
사용이 중단되었습니다. 대신 `save`로 타입을 직접 변경하세요.
