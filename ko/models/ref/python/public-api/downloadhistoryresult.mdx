...
실행의 히스토리 내보내기 다운로드 결과입니다. 

**Attributes:**

- `paths`:  다운로드된 히스토리 파일들의 경로입니다. 
 - `errors`:  히스토리 파일을 다운로드하는 동안 발생한 오류들의 딕셔너리입니다. 
 - `contains_live_data`:  실행에 실시간 데이터가 포함되어 있는지 여부입니다. 
 - `not yet exported to parquet files`: 아직 파케이(parquet) 파일로 내보내지 않은 데이터가 있는지 여부입니다. 
