...
Reports는 `BetaReport` 객체를 지연 평가 방식으로 순회하는 반복자입니다.
...
* `name` (str, 선택 사항):  필터링할 리포트 이름입니다. `None`이면 모든 리포트를 가져옵니다.
* `entity` (str, 선택 사항):  프로젝트의 엔터티 이름입니다. 지정하지 않으면 프로젝트의 엔터티가 기본값입니다.
...
페이지네이션을 위한 GraphQL 쿼리 변수를 업데이트합니다.
