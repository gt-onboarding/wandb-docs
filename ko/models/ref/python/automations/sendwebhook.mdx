...
* `integration_id` (str): 요청을 보내는 데 사용할 webhook integration의 ID입니다.
* `request_payload` (Optional[Annotated]): webhook 요청에서 전송할 페이로드로, 템플릿 변수를 포함할 수 있습니다.
...
지정된 웹후크 통합으로 전송하는 웹후크 액션을 정의합니다.
