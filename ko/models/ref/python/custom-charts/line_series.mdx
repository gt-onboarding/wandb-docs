...
**Examples:**
모든 y 시리즈를 동일한 x 값에 대해 플롯하는 단일 x 배열을 로깅하는 예:
...
이 예제에서는 모든 `ys` 시리즈에 하나의 `xs` 시리즈(공유 x값)를 사용합니다. 이렇게 하면 각 y 시리즈가 동일한 x값(0-9)에 대해 플롯됩니다.

각 y 시리즈가 자체 x 배열에 대해 플롯되도록 여러 x 배열을 로깅하기:
...
이 예제에서는 각 y 시리즈가 고유한 x 시리즈에 대해 그려집니다. 이를 통해 데이터 시리즈마다 x 값이 일정하지 않은 경우에도 더 유연하게 처리할 수 있습니다.
